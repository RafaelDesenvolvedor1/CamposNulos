...
# Permite o usuário navegar e escolher qual pasta quer processar
...
# Adicionada opção de voltar ao menu principal
...
# Fluxo genérico para processar qualquer limpeza de arquivo com isolamento por subdiretório
def processarArquivo(caminho_base, coluna):
    ...
    pasta_modificado_base = pasta_selecionada / "mod"

    # CAPTURA CENTRALIZADA: Solicita a transaction logo no início para evitar repetições
    transaction_uid = input("Informe a transaction: ").strip()
    if not transaction_uid:
        print("❌ Operação abortada. O UID da transaction não pode ser vazio.")
        return

    # Isola o destino final modificado dentro de uma subpasta com o ID da transaction
    pasta_modificado_final = pasta_modificado_base / transaction_uid

    # 2.1 Buscar o arquivo da baixa no bucket
    match menu.menu_dinamico(['Sim', 'Não'], 'Quer buscar o arquivo no bucket?'):
        case 0:
            menu.escolherCliente(destino=pasta_original, transaction_uid=transaction_uid)
    ...
    # MODIFICAÇÃO: O caminho de saída agora aponta para dentro do subdiretório isolado
    path_saida = pasta_modificado_final / nome_selecionado

    # Garante que a árvore física da subpasta mod/transaction_uid exista antes de salvar
    pasta_modificado_final.mkdir(parents=True, exist_ok=True)
    ...
    # 5. Gerar a soma e o relatório TXT dentro da subpasta isolada
    total = somarColuna(df_limpo)
    criarArquivoTXT(pasta_modificado_final, nome_selecionado, total)
    ...
    print(f"\n✅ Sucesso! Arquivo gerado e salvo na pasta 'mod/{transaction_uid}'.")

    # 7. Enviar para o bucket reaproveitando o UID capturado
    match menu.menu_dinamico(['Sim', 'Não'], 'Quer enviar o arquivo para o bucket?'):
        case 0:
            menu.enviarParaCliente(orig=pasta_modificado_final, transaction_uid=transaction_uid)
